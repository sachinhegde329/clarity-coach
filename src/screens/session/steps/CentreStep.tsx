...
export function CentreStep({
  ...
  if (unifiedDesign) {
    ...
    if (sn === 1) {
      const totalRounds = 4;
      const currentRound = breathing
        ? Math.min(Math.floor(guidedElapsedMs / Math.max(cycleMs, 1)) + 1, totalRounds)
        : 0;
      const phaseIsInhale = breathing && phaseIndex === 0;
        ...
        <View style={[styles.wrap, styles.unifiedWrap]}>
          <DottedStageBackground>
            <View style={{ width: "100%", gap: spacing.lg }}>
              ...
              {breathing && centreState === "GUIDED_ACTIVITY" ? (
                <View style={{ alignItems: "center", gap: spacing.sm }}>
                  <View style={{ flexDirection: "row", gap: 6, width: "100%", maxWidth: 240 }}>
                    {Array.from({ length: totalRounds }).map((_, i) => {
                      const filled = i + 1 < currentRound;
                        ...
                        <View
                          ...
                          style={{
                            ...
                            backgroundColor: filled
                              ? palette.line
                              : active && centreState === "GUIDED_ACTIVITY"
                                ? palette.lineSoft
                                : "transparent",
                          }}
                        />
                  ...
                  <MonoText style={{ color: palette.inkMuted, letterSpacing: 2, fontSize: 10 }}>
                    ROUND {currentRound} / {totalRounds}
                  </MonoText>
                </View>
              ) : null}

              <View style={{ alignItems: "center", minHeight: 300, justifyContent: "center" }}>
                <View style={styles.orbWrap}>
                  <Animated.View
                    style={[
                      flowStyles.centreBreathRing,
                      {
                        transform: [
                          {
                            scale: organicArrive.interpolate({
                              inputRange: [0, 1],
                              outputRange: [0.9, 1],
                            }),
                          },
                  ...
                  {breathing && centreState === "GUIDED_ACTIVITY" ? (
                    <View style={{ position: "absolute", bottom: -40, flexDirection: "row", gap: 10 }}>
                      <View style={[styles.organicTimerPill, phaseIsInhale ? styles.organicTimerPillActive : styles.organicTimerPillIdle]}>
                        <Text style={[styles.organicTimerText, phaseIsInhale ? styles.organicTimerTextActive : styles.organicTimerTextIdle]}>
                          4s In
                        </Text>
                      </View>
                      <View style={[styles.organicTimerPill, !phaseIsInhale ? styles.organicTimerPillActive : styles.organicTimerPillIdle]}>
              ...
              {config.organicOrb ? (
                <Animated.Text
                  style={[styles.instruction, { opacity: organicInstructionOpacity }]}
                  numberOfLines={1}
                >
                  {instruction}
                </Animated.Text>
              ...
              <View style={flowStyles.centreInstructionBlock}>
                ...
                {secondaryLines ? (
                  <Text style={flowStyles.stagePullQuote}>{secondaryLines}</Text>
                ) : null}
              </View>
            </View>
          </DottedStageBackground>
